...
let leastEngaged = [];
let mostEngaged = [];
let leastEngagedSenate = document.getElementById("senate-least-engaged");
let mostEngagedSenate = document.getElementById("senate-most-engaged");
let fieldsInserted = ["first_name", "missed_votes", "missed_votes_pct"];
let stats = {};

init(senateMembers);

function init(memberList) {
  generatePartyLists(memberList);
  setStats();
  printDataAtGlance();
  sortMembers(memberList);
  generateLeastEngaged(memberList);
  printLeastEngagedSenate();
  generateMostEngaged(memberList);
  printMostEngagedSenate();
}
...
function printLeastEngagedSenate() {
  for (let i = 0; i < leastEngaged.length; i++) {
    ...
    leastEngagedSenate.appendChild(newTr);
  }
}

function printMostEngagedSenate() {
  for (let i = 0; i < mostEngaged.length; i++) {
    ...
    mostEngagedSenate.appendChild(newTr);
  }
}